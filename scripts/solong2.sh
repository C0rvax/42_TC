...
for i in "${ARG[@]}"; do # on parcours la liste d'argument ARG
	...
	./so_long ./maps/invalid/$i 2>&1 >/dev/null | grep "Error" | wc -l >err # on écrit combien de ligne contient Error dans un fichier tmp
	error=$(cat err)                                                        # on recupère le resultat de tmp dans une variable : grep

	valgrind --leak-check=full ./so_long ./maps/invalid/$i 2>&1 >/dev/null | grep "no leaks are possible" | wc -l >leak
	leaks=$(cat leak)
