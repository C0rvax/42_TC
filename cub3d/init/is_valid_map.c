...
/*   Updated: 2024/06/05 01:03:14 by aduvilla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (map && map[i])
	{
		...
		while (map[i][j])
		{
			...
			if (map[i][j] == 'N' && map[i][j + 1] == 'O')
				edit_wall(cub->scene->sprite.north, map[i]);
			else if (map[i][j] == 'S' && map[i][j + 1] == 'O')
				edit_wall(cub->scene->sprite.south, map[i]);
			else if (map[i][j] == 'W' && map[i][j + 1] == 'E')
				edit_wall(cub->scene->sprite.west, map[i]);
			else if (map[i][j] == 'E' && map[i][j + 1] == 'A')
				edit_wall(cub->scene->sprite.east, map[i]);
			else if (map[i] == 'F' && map[i][j + 1] == ' ')
				test_n(map, i);
			else if (map[i] == 'C')
				test_n(map, i);
			else
...
int	init_cub(char *map, t_cub *cub)
{
	...
	if (check_file(map, ".cub"))
		return (1);
	line = get_all_lines(map);
	if (!line)
		return (perror("Error\n"), 1);
	tab = ft_split(line, '\n');
	if (!tab)
